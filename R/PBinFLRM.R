#' @title Paired bootstrap in functional linear regression models
#' 
#' @description 
...
PBinFLRM = function(X, Y, X0, tGrid, kn_vec, hn_vec, gn_vec, M_bts=1000, ap=0.05){
  ...
  resPBouter = BTSinFLRM:::PBouter(
    X, Y, X0, tGrid, 
    res$est$betaHat, res$est$UHat, res$CLThetero$sxHat,
    res$est$Y0Hat, res$est$Y0Hat_trunc_hg,
    res$estTilde$YTilde, res$estTilde$betaTilde, 
    res$estTilde$UTilde, res$estTilde$Y0Tilde,
    res$STAThetero$stat_nonstd, res$STAThetero$stat_Sstd,
    kn_vec, hn_vec, gn_vec, M_bts, ap)
  dimnames(resPBouter$intervals) = list(
    paste0("kn=", kn_vec),
    paste0("hn=", hn_vec),
    paste0("gn=", gn_vec)
  )
  dimnames(resPBouter$pVal) = list(
    paste0("kn=", kn_vec),
    paste0("hn=", hn_vec),
    paste0("gn=", gn_vec)
  )
  ...
  for(ii_k in 1:length(kn_vec)){
    for(ii_h in 1:length(hn_vec)){
      for(ii_g in 1:length(gn_vec)){
        # intervals
        ress = abind::abind(
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,1:2], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,3:4], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,5:6], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,7:8], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,9:10], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,11:12], dim = c(n0, 2, 2) ),
          array( resPBouter$intervals[[ii_k, ii_h, ii_g]][,,13:14], dim = c(n0, 2, 2) ),
          along=4
        )
        dimnames(ress) = list(
          paste0("X0_", 1:n0),
        ...
        resPBouter$intervals[[ii_k,ii_h,ii_g]] = ress
        
        
        # p-values
        resss = abind::abind(
          resPBouter$pVal[[ii_k,ii_h,ii_g]][1:3,],
          resPBouter$pVal[[ii_k,ii_h,ii_g]][4:6,],
          along=3
        )
        dimnames(resss) = list(
          c("nonstd", "std1", "std2"),
        ...
        resPBouter$pVal[[ii_k,ii_h,ii_g]] = resss
  ...
  resPB = list(
    est = res$est$betaHat,
    CLT = CLT_hetero,
    PBintervals = resPBouter$intervals,
    PBpvalues = resPBouter$pVal
  )
  return(resPB)
